...
`
    ...
    rendered += `  Shepherd type & role: ${imageMetadata.deploymentType} - ${imageMetadata.deployerRole}
...
Git url - ${imageMetadata.gitUrl}
 ${imageMetadata.kubeConfigB64 && '\nK8S deployment files:' + renderFileList(imageMetadata.kubeDeploymentFiles) || ''}
`
...
${imageMetadata.environmentVariablesExpansionString && 'environment variables expansion string'} `
...
`
    ...
    if(imageMetadata.hyperlinks && imageMetadata.hyperlinks.length){
        rendered += 'Links:\n        '
        rendered += imageMetadata.hyperlinks.map((href)=>{
