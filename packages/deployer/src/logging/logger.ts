...
function formatLogWithPrefix(logString: string, logLineLength: number, prefixWithContext: (logLine: string) => string, consoleInstance: any) {
  logString
    .toString()
    .split("\n")
...
function renderLogStringWithColoredPrefix(options: TLoggerOptions, prefixWithContext: (logLine: string) => string, logPrefixColor: chalk.ChalkFunction | undefined, logString: string, consoleInstance: any, prefix: string) {
  ...
  prefixWithContext = (logLine: string) => `${logPrefixColor && logPrefixColor(prefixString) || prefixString} ${logLine}`
  ...
  let buildLogger: ILog = {
    info(logString: string, logContext: TLogContext=options.defaultContext || DEFAULT_LOG_CONTEXT) {
      ...
      if(logContext.prefix){
        ...
        prefixWithContext = renderLogStringWithColoredPrefix(options, prefixWithContext, logPrefixColor, logString, consoleInstance, logContext.prefix)
      } else {
