...
function compareActualVsExpected(expectedFileName, actualFileName) {
    ...
    if (containsDifference(difference)) {
        expect().fail("Expected file " + expectedFileName + " differs from actual file " + actualFileName + "\n" + renderDifferences(difference));
    }
}
...
module.exports = {
    ...
    execute: function (command, args, options) {
        ...
        exec.extendedExec(command, args, options, function (err, errCode, stdout) {
            if(execution.expectedExitCode){
                ...
                expect().fail("Error invoking : " + command + " with arguments " + JSON.stringify(args) + "\nStdout: \n" + stdout + "\nError output:\n" + err + "\n. ErrorCode:" + errCode);
            }
        }, function (output) {
            ...
            checkExpectations(){
                ...
                if (execution.actualFromStdout) {
                    ...
                    if (fs.existsSync(execution.expectedStdOutput)) {
                        expectedOutput = fs.readFileSync(execution.expectedStdOutput, 'utf-8');
                    } else {
                    ...
                    let actualIsDir = fs.lstatSync(execution.actualOutputFileOrDir).isDirectory();
                    ...
                    let expectedIsDir = fs.lstatSync(execution.expectedOutputFileOrDir).isDirectory();
                    if (( expectedIsDir || actualIsDir ) && !( expectedIsDir && actualIsDir )) {
                        expect().fail("Both expected and actual must be a directory or a file");
                    }
                    if (expectedIsDir) {
                        let expectedFiles = _.sortedUniq(fs.readdirSync(execution.expectedOutputFileOrDir));
                        let actualFiles = _.sortedUniq(fs.readdirSync(execution.actualOutputFileOrDir));
                        ...
                        if (containsDifference(difference)) {
                            expect().fail("Expected directory contents " + execution.expectedOutputFileOrDir + " differs from actual dir contents " + execution.actualOutputFileOrDir + "\n" + renderDifferences(difference));
                        }
                        _.each(actualFiles, function (file) {
                            compareActualVsExpected(execution.expectedOutputFileOrDir + "/" + file, execution.actualOutputFileOrDir + "/" + file);
                        });
