...
describe("herd.yaml loading", function() {
        ...
        addDeployment(deployment) {
          return new Promise(function(resolve, reject) {
            ...
            if (deployment.type === "k8s") {
              releasePlan.addedK8sDeployments[
                deployment.identifier
              ] = deployment
            } else if (deployment.type === "deployer") {
              releasePlan.addedDockerDeployers[
                deployment.identifier
              ] = deployment
            }
          ...
          getImageLabels(imageDef) {
            ...
            if (fs.existsSync(dockerImageMetadataFile)) {
              ...
              return Promise.reject(
                new Error(
                  `dockerImageMetadataFile ${dockerImageMetadataFile} not found in testdata`
                )
              )
            }
          },
  ...
  it("should load herd.yaml", function() {
    return loader
      .loadHerd(__dirname + "/testdata/happypath/herd.yaml")
      .then(function(plan) {
    ...
    loader
      ...
      .catch(function(error) {
        expect(error).to.contain(
          "/testdata/does-not-exist.yaml does not exist!"
        )
      })
  })
  ...
  describe("directory execution plan", function() {
    ...
    it("should add k8s deployment found in scanned directory", function() {
      ...
      expect(
        loadedPlan.addedK8sDeployments["Namespace_monitors"].origin
      ).to.contain("testdata/happypath/namespaces")
    })
    ...
    it("should have herd name", function() {
      ...
      expect(
        loadedPlan.addedK8sDeployments["Namespace_monitors"].herdName
      ).to.contain("kube-config")
      expect(
        loadedPlan.addedK8sDeployments["Namespace_monitors"].herdName
      ).to.contain("testdata/happypath/namespaces")
    })
  ...
  describe("images execution plan", function() {
    ...
    it("should base64decode and untar deployment files under file path", function() {
      expect(
        loadedPlan.addedK8sDeployments["Service_www-icelandair-com"].origin
      ).to.equal("testenvimage:0.0.0:kube.config.tar.base64")
    })

    it("should extract herdName from herd.yaml", function() {
      expect(
        loadedPlan.addedK8sDeployments["Service_www-icelandair-com"].herdName
      ).to.equal("test-image")
    })

    it("should modify deployment documents and file under deployments under k8s service identity", function() {
      expect(
        loadedPlan.addedK8sDeployments["Service_www-icelandair-com"].descriptor
      ).not.to.contain("${EXPORT2}")
    })
    ...
    it("should apply k8s deployment-time cluster policy", function() {
      ...
      expect(
        loadedPlan.addedK8sDeployments["Deployment_www-icelandair-com"]
          .descriptor
      ).to.contain("25m")
    })
  ...
  describe("deployer execution plan", function() {
    ...
    it("should load deployer plan by migration image reference", function() {
      expect(
        loadedPlan.addedDockerDeployers["testenvimage-migrations:0.0.0"]
          .dockerParameters
      ).to.contain("testenvimage-migrations:0.0.0")
      expect(Object.keys(loadedPlan.addedDockerDeployers)).to.contain(
  ...
  xdescribe("SLOW TEST: non-existing image", function() {
    ...
    beforeEach(function() {
      return loader
        .loadHerd(__dirname + "/testdata/nonexistingimage/herd.yaml")
        .then(function(plan) {
          loadedPlan = plan
        })
    })
