...
describe('k8s deployment file directory structure release plan loader', function () {
    ...
    describe('successful load', function () {
        ...
        it('should add origin to plan', function () {
            expect(plan.addedK8sDeployments['Service_www-icelandair-com-fromdir'].origin).to.contain('/testdata/deployment-dirs/www-icelandair-com');
        });

        it('should result in exactly those plans', function () {
            expect(Object.keys(plan.addedK8sDeployments).sort().join(',')).to.equal('Deployment_www-icelandair-com-deleted,Deployment_www-icelandair-com-fromdir,Namespace_monitors,Service_www-icelandair-com-fromdir,Service_www-icelandair-com-internal-fromdir');
        });

        it('all versions should be immutable', function () {
            expect(plan.addedK8sDeployments['Service_www-icelandair-com-fromdir'].version).to.equal('immutable');
        });

        it('default operation should be apply', function () {
            expect(plan.addedK8sDeployments['Service_www-icelandair-com-fromdir'].operation).to.equal('apply');
        });

        it('operation of file in directory with delete marker should be delete', function () {
            expect(plan.addedK8sDeployments['Namespace_monitors'].operation).to.equal('delete');
        });
        ...
        it('should add k8s deployment to plan for each file under k8s identifier', function () {
            expect(plan.addedK8sDeployments['Service_www-icelandair-com-fromdir']).not.to.equal(undefined);
        });
        ...
        it('should mark deployment for delete if folder_name_image variable is set and empty', function () {
            expect(plan.addedK8sDeployments['Deployment_www-icelandair-com-deleted'].operation).to.equal('delete')
        });
