                ...
                list.forEach(function (unresolvedPath) {
                    ...
                    fs.stat(resolvedPath, function (err, stat) {
                        ...
                        if (stat && stat.isDirectory()) {
                            ...
                            planPromises.push(scanDir(subDir).then(function(plans){
                                if (!--pending) scanCompleted();
                                return plans;
                            ...
                            try {
                                if (isDeploymentFile(resolvedPath)) {
                                    ...
                                    planPromises.push(calculateFileDeploymentPlan(resolvedPath)
                                        .then(function (plan) {
                                            dir.files[baseName] = {
                                            ...
                                            pending = -1;
                                            reject('Storing deployment state, for file ' + resolvedPath + ':\n' + e);
                                        }));
                                } else {
                                    if (!--pending) scanCompleted();
                                }
