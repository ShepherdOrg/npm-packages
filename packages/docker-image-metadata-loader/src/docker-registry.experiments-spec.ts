...
describe("Get docker info from local registry using superagent", function() {
  ...
  it("Should list all shepherd labelled images", () => {
    ...
    return agent
      ...
      .then(imagesWithTags => {
        const imageManifestPromises = imagesWithTags.map(
          (imageWithTag: any) => {
            console.debug(
              "Getting manifest for ",
              imageWithTag.displayName,
              imageWithTag.tags[0]
            )
      ...
      .then(imagesWithManifests => {
        ...
        return imagesWithManifests.filter(
          current => current && current["shepherd.displayName"]
        )
      })
      .then(filtered => {
