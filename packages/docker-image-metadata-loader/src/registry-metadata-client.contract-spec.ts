...
describe("Registry metadata API against localhost on http", function() {
  ...
  it("should have expected property names on manifest", () => {
    return api.getImageManifest("localhost:5000/shepherd", "latest").then((manifest: any) => {
  ...
  it("should give meaningful error on non-existing image", () => {
    return api.getImageManifestLabels("localhost:5000/nowayjose", "latest").catch(err => {
      expect(err.message).to.equal("localhost:5000/nowayjose:latest Not Found");
    });
  });
});
...
describe("Registry metadata API against localhost with basicauth on https", function() {
  ...
  it("should have expected property names on manifest", () => {
    return api.getImageManifest(`${dockerHost}/shepherd`, "latest").then((manifest: any) => {
      const expectedPropNames = [
        "schemaVersion",
        "name",
        "tag",
