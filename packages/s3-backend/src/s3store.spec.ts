...
describe("s3 object store backend", function() {
  let store
    ...
    return new Promise(function(resolve, reject) {
      store
        ...
        .then(function(_numberOfDeployments) {
          store
            .set("postgres/deployment", { is: "awesomeToo" })
  ...
  it("should set object twice", function() {
    return new Promise(function(resolve, reject) {
      store
        ...
        .then(function(_numberOfDeployments) {
          store
            .set("postgres/deployment", { is: "awesomerrrr" })
            .then(_writeOne => {
              store
                .set("postgres/deployment", { is: "awesome" })
                .then(_writeTwoResult => {
                  store
                    .get("postgres/deployment")
                    .then(function(retrievedKeyPair) {
                      expect(retrievedKeyPair.key).to.eql("postgres/deployment")
    ...
    return new Promise(function(resolve, reject) {
      store
        ...
        .then(() => {
          store
            .disconnect()
            .then(function(_retrievedKeyPair) {
              resolve()
            })
            .catch(function(setError) {
              reject(setError)
